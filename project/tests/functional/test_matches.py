...
def test_single_match_db(test_app):
    ...
    client = test_app.test_client()
    response = client.get('/matches/match?squad_name=Liverpool')

    assert response.status_code == 200
    assert response.headers['Content-Type'] == "application/json"
    assert response.json == [{'first_squad_name': 'Liverpool',
...
def test_single_match_by_round(test_app):
    """Start with a single match in the database and get this match."""
    client = test_app.test_client()
    response = client.get('/matches/match?round_name=Week 1')
...
def test_get_matches(test_app):
    ...
    assert response.status_code == 200
    assert response.headers['Content-Type'] == "application/json"
    assert response.json['matches'][0]['match_id'] == 1
    ...
    assert response.json['matches'][1]['match_id'] == 2
    assert response.json['matches'][2]['match_id'] == 3


def test_get_one_match(test_app):
    """Get one team by match name."""
    client = test_app.test_client()
    response = client.get('/matches/match?squad_name=Burnley')

    assert response.status_code == 200
    assert response.headers['Content-Type'] == "application/json"
    assert response.json == [{'first_squad_name': 'Manchester City',
                              'first_squad_points': 0,
                              'first_squad_score': 0,
                              'match_date': '2019-08-10',
                              'round_name': 'Week 2',
...
def test_get_one_team_multiple_matches(test_app):
    ...
    assert response.status_code == 200
    assert response.headers['Content-Type'] == "application/json"
    assert response.json == [{'round_name': 'Week 1',
                              'match_date': '2019-08-10',
...
def test_get_non_existing_match(test_app):
    ...
    client = test_app.test_client()
    response = client.get('/matches/match?squad_name=non_existing')
...
def test_get_matches_by_complex_name(test_app):
    ...
    client = test_app.test_client()
    response = client.get('/matches/match?squad_name=Manchester-City')

    assert response.status_code == 200
    assert response.headers['Content-Type'] == "application/json"
    print('response.json', response.json)
    assert response.json[0]['second_squad_name'] == 'Manchester City'
    assert response.json[1]['match_id'] == 2
    assert response.json[2]['match_id'] == 3
